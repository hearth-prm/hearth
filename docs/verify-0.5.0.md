...
Most of this is manual because the parts that can genuinely break are the parts a
fake client cannot prove: whether Google's contact-group API behaves as assumed, and
whether a real spreadsheet round-trips through a real file.
...
| # | Test | Expected |
|---|---|---|
| 6.1 | Export everything, then import that file unchanged | Preview shows **all updates, zero creates** |
...
| 7.10 | A file with only `Hearth ID` and `Labels` | Preview warns other contact details are kept; after import, **phone numbers survive** |
...
| 8.5 | A row whose `Hearth ID` is a contact B has **not** shared | Does **not** touch it — becomes a new contact of A's, with a warning |

...
nothing lost; 878 automated checks. See [CHANGELOG.md](CHANGELOG.md) for what landed.
...
built app through 878 checks, and `npm run e2e:google` runs the Google-facing half
...
`docker-compose` is on your PATH, so having podman is not the same as having a current Compose.
